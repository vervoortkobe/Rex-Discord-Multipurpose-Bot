...
module.exports.run = async (client, message, args) => {
    ...
    if(!message.member.hasPermission('KICK_MEMBERS')) return message.channel.send(`❌ | Couldn't mute the user, because you do not have the correct permissions (KICK_MEMBERS) to do this!`);
    ...
    const mutePmEmbed = new Discord.RichEmbed()
    .setColor(0xff0000)
    .setTitle(`⛑️ | ${client.user.username} Muted`)
    .setDescription(`You just got muted on \`${message.guild.name}\`, because of \`${reason}\`!`)
    ...
    .addField(`MEMBER_MUTE`, `${message.author.tag} just **muted ${member.user.tag}** because of: **${reason}**!`)
    .setFooter(`© ${client.user.username} was made by Tsunami#6271`)
