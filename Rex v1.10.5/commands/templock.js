...
module.exports.run = async (client, message, args) => {
    ...
    .setDescription(`Usage: **${prefix}templock <time> <m/h/d>**`)
    .setFooter(`© ${client.user.username} was made by Tsunami#6271`)
    if(!args[0]) return message.channel.send(templockUsageEmbed);

    const atemplockUsageEmbed = new Discord.RichEmbed()
    .setColor(0xff0000)
    .setTitle(`⚙️ | ${client.user.username} Templock`)
    .setDescription(`Usage: **${prefix}templock <time> <m/h/d>**`)
    .setFooter(`© ${client.user.username} was made by Tsunami#6271`)
    if(!args[1]) return message.channel.send(atemplockUsageEmbed);
