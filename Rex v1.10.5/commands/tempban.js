...
module.exports.run = async (client, message, args) => {
    ...
    if(!args[1]) return message.channel.send(tempbanUsageEmbed);

    const atempbanUsageEmbed = new Discord.RichEmbed()
    .setColor(0xff0000)
    .setTitle(`⚙️ | ${client.user.username} Tempban`)
    .setDescription(`Usage: **${prefix}tempban <user> <time> <m/h/d> <reason>**`)
    .setFooter(`© ${client.user.username} was made by Tsunami#6271`)
    if(!args[2]) return message.channel.send(atempbanUsageEmbed);
