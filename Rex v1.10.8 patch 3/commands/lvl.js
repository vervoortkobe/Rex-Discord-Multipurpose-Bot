...
module.exports.run = async (client, message, args) => {
    ...
    const lvlmode = `Normal`;

    if(member.id === config.donator1 || 
       member.id === config.donator2 || 
       member.id === config.donator3 || 
       member.id === config.donator4 || 
       member.id === config.donator5
       ) {
      lvlmode = `Donator (2x)`
       }
      
    if(member.id === config.premium1 || 
       member.id === config.premium2 || 
       member.id === config.premium3 || 
       member.id === config.premium4 || 
       member.id === config.premium5
       ) {
      lvlmode = `Premium (2x)`
       }
