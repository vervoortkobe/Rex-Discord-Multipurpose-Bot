...
module.exports.run = async (client, message, args) => {
    ...
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.KICK_MEMBERS)) {
      ...
      .setColor(0xf04947)
      .setDescription(`${x} **|** ***I couldn't kick this member, because I don't have kick permissions!***`)
    ...
    if(!message.member.permissions.has(Discord.Permissions.FLAGS.KICK_MEMBERS)) {
      const errorEmbed = new Discord.MessageEmbed()
      .setColor(0xf04947)
      .setDescription(`${x} **|** ***I couldn't kick this member, you don't have the correct permissions (KICK_MEMBERS) to do this!***`)
      return message.channel.send({ embeds: [ errorEmbed ]});
    }
    ...
    if(!args[0]) {
      ...
      .setColor(0xf04947)
      .setAuthor(`⚙️ | Kick`)
      .setDescription(`${x} **|** ***Usage: ${prefix}kick <@member/member ID> [reason]***`)
    ...
    if(member.bot) {
      const errorEmbed = new Discord.MessageEmbed()
      .setColor(0xf04947)
      .setDescription(`${x} **|** ***You can't kick a bot!***`)
      return message.channel.send({ embeds: [ errorEmbed ]});
    }
    ...
    member.send({ embeds: [ kickPmEmbed ]})
