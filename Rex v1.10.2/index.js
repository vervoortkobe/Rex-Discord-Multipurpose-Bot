...
fs.readdir("./commands/", (err, files) => {
  ...
  client.on("guildMemberAdd", member => {
    ...
    let joinChannel = member.guild.channels.find(c => c.name === `join-leave`);
      ...
      .addField(`${member} just joined ${member.guild.name}!`, `We now have ${message.guild.members.size} members!`)
      .setFooter(`© ${client.user.username} was made by Tsunami#6271`)
      .setTimestamp()    
    leaveChannel.send(joinEmbed);
  });
  ...
  client.on("guildMemberRemove", member => {
    ...
    let leaveChannel = member.guild.channels.find(c => c.name === `join-leave`);
      ...
      .setThumbnail(member.avatarURL)
      .addField(`${member} just left ${member.guild.name}!`, `We now have ${message.guild.members.size} members!`)
      .setFooter(`© ${client.user.username} was made by Tsunami#6271`)
      .setTimestamp()    
