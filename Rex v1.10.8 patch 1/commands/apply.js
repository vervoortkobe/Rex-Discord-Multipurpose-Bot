...
module.exports.run = async (client, message, args) => {
    ...
    if(!args[0]) {
      ...
      .setDescription(`Usage: **${prefix}apply <your apply>**`)
      .setFooter(`© ${client.user.username} was made by Tsunami#6271`)
      return message.channel.send(applyUsageEmbed);
    }

    const applyChannel = message.guild.channels.find(c => c.name === `rex-apply`);
    if(!applyChannel) return message.channel.send(`❌ | I couldn't submit your apply, because the owner of this server doesn't want to let members apply or forgot create an apply channel! To do this, pls send this command: \`${prefix}rex-apply\`.`);
