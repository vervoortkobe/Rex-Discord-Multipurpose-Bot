...
fs.readdir("./commands/", (err, files) => {
  ...
  client.on('guildMemberAdd', member => {
    ...
    const invitejoinlogChannel = member.guild.channels.find(c => c.name === `rex-invitelogs`);
      if(!invitejoinlogChannel) return;
      ...
      invitejoinlogChannel.send(`**${member.user.tag} just joined** using invite code ${invite.code} **from ${inviter.tag}**!\nThis invite was used **${invite.uses} times** since its creation.`);
    });
  });
