...
module.exports.run = async (client, message, args) => {
    ...
    if(!tempmuteTime || !timeDuration) {
      const tempmuteUsageEmbed = new Discord.RichEmbed()
      .setColor(0xff0000)
      .setTitle(`⚙️ | ${client.user.username} Tempmute`)
      .setDescription(`Usage: **${prefix}tempmute <user> <time> <m/h/d/w> <reason>**`)
    ...
    if(!timeDuration === `m` || !timeDuration === `h` || !timeDuration === `d` || !timeDuration === `w`) return message.channel.send(tempmuteUsageEmbed);
    ...
    const tempmutePmEmbed = new Discord.RichEmbed()
    .setColor(0xff0000)
    .setTitle(`⛑️ | ${client.user.username} Tempmuted`)
    .setDescription(`You just got tempmuted on \`${message.guild.name}\` for: \`${args[1]}${args[2]}\`, because of \`${reason}\`!`)
